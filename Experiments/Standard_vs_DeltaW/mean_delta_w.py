...
class MeanDeltaW(nn.Module):
    """
    Generate low-rank ΔW updates from the masked mean
    representation of the current sequence at every hop.
    """
    ...
    def forward(self, ids, mask):
        ...
        for block in self.blks:
            deltas = self.hyper(seq_mean(x, mask))
...
def main():
    ...
    print("\n" + "=" * 65)
    print(" HYPERNET: MEAN → ΔW STRICT EVALUATION ")
    print("=" * 65)
    ...
    for task_name, splits in datasets.items():
        ...
        for seed in SEEDS:
            ...
            model = MeanDeltaW(cfg).to(cfg.device)
